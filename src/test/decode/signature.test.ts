...
import { verifyPngSignature } from '../../decode/decoder.js';
...
describe('verifyPngSignature', () => {
  it('should throw when the data doesn\'t match the fixed 8-byte header', () => {
    throws(() => {
      verifyPngSignature({ view: dataViewFromArray([0x41, 0x4D]), warnings: [], info: [], metadata: [], options: null!, parsedChunks: new Set() });
    }, new Error('Not enough bytes in file for png signature (2)'));
    throws(() => {
      verifyPngSignature({ view: dataViewFromArray([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]), warnings: [], info: [], metadata: [], options: null!, parsedChunks: new Set() });
  ...
  it('should verify the header of valid png suite entries', async () => {
    ...
    for (const file of testFiles) {
      ...
      if (file.endsWith('png') && !file.startsWith('x')) {
        verifyPngSignature({ view: await dataViewFromFile(join(pngSuiteRoot, file)), warnings: [], info: [], metadata: [], options: null!, parsedChunks: new Set() });
      }
    }
  });
  it('should throw for corrupt headers in png suite', async () => {
    ...
    for (const file of testFiles) {
      ...
      throws(() => verifyPngSignature({ view, warnings: [], info: [], metadata: [], options: null!, parsedChunks: new Set() }));
    }
  });
});
