...
import { assertChunkDataLengthEquals, assertChunkPrecedes, DecodeWarning } from '../assert.js';
    ...
    case ColorType.Indexed:
      ...
      if (ctx.palette) {
        if (chunk.dataLength > ctx.palette.size) {
          throw new DecodeWarning(chunk, `Invalid data length for color type ${header.colorType}: ${chunk.dataLength} > ${ctx.palette.size}`, chunk.offset + ChunkPartByteLength.Length + ChunkPartByteLength.Type);
        }
      }
      break;
    case ColorType.GrayacaleAndAlpha:
    case ColorType.TruecolorAndAlpha:
      throw new DecodeWarning(chunk, `Chunk invalid when color type has alpha (${header.colorType})`, chunk.offset + ChunkPartByteLength.Length + ChunkPartByteLength.Type);
  }
