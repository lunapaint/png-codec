...
import { assertChunkDataLengthEquals, assertChunkPrecedes, assertChunkSinglular, ChunkError, handleWarning } from '../assert.js';
...
export function parseChunk(ctx: IDecodeContext, header: IPngHeaderDetails, chunk: IPngChunk): IPngMetadataGamma {
  ...
  if (value === 0) {
    handleWarning(ctx, new ChunkError(chunk, 'A value of 0 is meaningless', offset));
  }
